...
const SideBarDrawer = ({ children, isOpen, onOpen, onClose, className }) => {
  ...
  return (
    <div id='sidebar-wrapper' className={' block lg:hidden top-0 ' + className}>
      <div
        id='sidebar-drawer'
        className={`${isOpen ? 'mr-0 w-72 visible' : '-mr-72 max-w-side invisible'} bg-stone-50/30 dark:bg-slate-900/30 backdrop-blur-md right-0 top-0 shadow-black shadow-lg flex flex-col duration-300 fixed h-full overflow-y-scroll scroll-hidden z-30`}>
        {children}
      </div>
      ...
      <div
        ...
        className={`${isOpen ? 'block' : 'hidden'} animate__animated animate__fadeIn fixed top-0 duration-300 left-0 z-20 w-full h-full bg-black/20 backdrop-blur-sm`}
      />
    </div>
  )
}
